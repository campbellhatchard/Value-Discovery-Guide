...
function fallbackResult(reason) {
  return {
    ...
    problem_complete: false, ask_other_problems: false, transition_statement: "", summary_statement: ""
  };
}
...
function cutoffDateMonthsAgo(months) { const d = new Date(); d.setMonth(d.getMonth() - months); return d.getTime(); }
async function fetchNewsSignals(companyInput) {
  const queries = [
    ...
    `"${companyInput}" maintenance OR service expansion OR facility`
  ];
  const all = [];
  ...
  const unique = uniqByLink(all).map(item => ({
    signal: item.title, date: formatDate(item.pubDate), source_name: item.source || '', source_url: item.link
  })).filter(item => {
    if (!item.date) return true;
    const t = new Date(item.date).getTime();
    return !isNaN(t) && t >= cutoff;
  });
  return sortSignalsByMostRecent(unique).slice(0, 12);
}
...
Return ONLY valid JSON in this exact format:
{
  ...
  "industry":"string",
  "inventory_challenges":{"warehouse":["string"],"manufacturing":["string"],"field_inventory":["string"]},
  "discovery_angles":["string"],
  "opening_hook":"string",
  "smart_first_questions":["string","string","string"],
  "likely_operational_triggers":[{"title":"string","why_occurring":"string","why_inferred":"string","why_it_matters":"string","suggested_question":"string"}]
}
...
- Adapt language to persona level.
...
- "likely_operational_triggers" must be dynamic to the company and industry.
`;
...
${JSON.stringify(newsItems, null, 2)}` }]}],
    ...
    const parsed = parseJsonFromText(extractOutputText(JSON.parse(raw)));
...
Return ONLY valid JSON in this exact shape:
{
  ...
  "updated_state":{"problem_location":"string","likely_cause":"string","business_impact":"string","why_it_matters":"string"},
...
Rules:
- Adapt wording and impact framing to persona level.
- Manager: workflow, delays, errors, day-to-day friction.
- Director: KPIs, efficiency, cross-functional performance, cost.
- Executive: revenue, risk, growth, customer impact, scale.
...
${latest_response || ''}` }]}],
    ...
    let parsed;
    try { parsed = parseJsonFromText(extractOutputText(JSON.parse(raw))); }
    catch (err) { parsed = fallbackResult("Fallback question used because the server could not safely interpret the AI response."); }
    res.json({ ok: true, result: parsed });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message || 'Unknown server error' });
  }
