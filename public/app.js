...
const el = {
  ...
  triggerPainGrid: document.getElementById('triggerPainGrid'),
  ...
  personaLevel: document.getElementById('personaLevel'),
...
function renderPainGrid(points){
  if (!el.painPointGrid) return;
  el.painPointGrid.innerHTML = '';
  points.forEach(title => {
    const id = 'std_' + title.toLowerCase().replace(/[^a-z0-9]+/g, '_');
    const card = document.createElement('div');
    card.className = 'pain-item';
    card.innerHTML = `<label><input type="checkbox" data-id="${id}"><div><div class="title">${escapeHtml(title)}</div><div class="desc">${DEFAULT_PAIN_DESC}</div></div></label>`;
...
function renderTriggerPainGrid() {
  if (!el.triggerPainGrid) return;
  el.triggerPainGrid.innerHTML = '';
  if (!state.companyTriggers.length) {
    el.triggerPainGrid.innerHTML = `<div class="notice">Analyze a company to populate likely operational triggers here.</div>`;
    return;
  }
  state.companyTriggers.forEach((trigger, idx) => {
    const id = 'trg_' + idx;
    const title = trigger.title || 'Untitled trigger';
    const desc = trigger.why_it_matters || trigger.why_occurring || 'Inferred from company research.';
    const card = document.createElement('div');
    card.className = 'pain-item';
    card.innerHTML = `<label><input type="checkbox" data-id="${id}"><div><div class="title">${escapeHtml(title)}</div><div class="desc">${escapeHtml(desc)}</div></div></label>`;
    card.querySelector('input').addEventListener('change', e => {
      toggleProblem({
        id,
        title,
        desc,
        meta: { type: 'trigger', why_occurring: trigger.why_occurring, why_inferred: trigger.why_inferred, why_it_matters: trigger.why_it_matters, suggested_question: trigger.suggested_question }
      }, e.target.checked);
    });
    el.triggerPainGrid.appendChild(card);
  });
}
...
function renderSelectedProblems(){
  if (!el.selectedList || !el.selectedCount) return;
  ...
  state.selectedProblems.forEach(p => {
    ...
    chip.innerHTML = `<span>${escapeHtml(p.title)}</span><button title="Remove">×</button>`;
    chip.querySelector('button').addEventListener('click', () => {
      ...
      renderSelectedProblems();
      if (!state.selectedProblems.length) newSession(false);
...
function renderActivePain(){
  if (!el.activePainCard || !state.currentProblem) return;
  let extra = '';
  if (state.currentProblem.meta?.type === 'trigger') {
    extra = `<div class="sub" style="margin-top:10px;">Inferred trigger selected from company research. Validate during discovery.</div>`;
  }
  el.activePainCard.classList.remove('hidden');
  el.activePainCard.innerHTML = `<div class="summary-box"><div class="title">Active pain point</div><div><strong>${escapeHtml(state.currentProblem.title)}</strong></div>${extra}<div class="sub" style="margin-top:6px;">The AI guide will adapt the next question based on what has already been uncovered and the selected audience level.</div></div>`;
  updateMetrics();
}
...
async function askNextQuestion(latestResponse){
  ...
  if (!state.config?.modelConfigured) {
    addQuestionCard('location', `Where does "${state.currentProblem.title}" show up most clearly in the operation today?`, '');
    return;
  }
...
function addQuestionCard(stage, question, rationale){
  if (!el.template || !el.conversation) return;
...
function updateMetrics(){
  ...
  [[el.mLocation,d.problem_location],[el.mCause,d.likely_cause],[el.mImpact,d.business_impact],[el.mUrgency,d.why_it_matters]].forEach(([node, value]) => {
    ...
    node.className = 'metric-value ' + s.cls;
  });
}
...
function finishSession(transition, summaryStatement){
  ...
  el.completionBox.innerHTML = `
    ...
    <div class="completion-actions">
      <button id="printBtn" class="btn btn-primary">Print Conversation</button>
      <button id="newSessionBtn" class="btn btn-secondary">Start New Session</button>
    </div>`;
  document.getElementById('printBtn')?.addEventListener('click', () => window.print());
  document.getElementById('newSessionBtn')?.addEventListener('click', () => newSession(true));
}
...
async function analyzeCompany() {
  ...
  if (!value || !el.companyResults) return;
  ...
  try {
    const res = await fetch('/api/company-insight', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ company_input: value, persona_level: state.persona })
    });
...
function newSession(clearCompany=true){
  ...
  if (el.selectedList) el.selectedList.innerHTML = '';
  if (el.selectedCount) el.selectedCount.textContent = '0 selected';
  if (el.conversation) el.conversation.innerHTML = '';
  if (el.completionBox) {
...
el.newSessionTop?.addEventListener('click', () => newSession(true));
...
el.personaLevel?.addEventListener('change', (e) => { state.persona = e.target.value; });
