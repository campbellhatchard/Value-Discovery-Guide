...
const SITE_INTEL_CACHE_TTL_MS = 7 * 24 * 60 * 60 * 1000;
const ASYNC_JOB_TTL_MS = 30 * 60 * 1000;
const ANALYSIS_CACHE = new Map();
const ANALYSIS_HISTORY = new Map();
...
const MAX_LINKED_DOMAINS = 4;
const MAX_PDFS_PER_RUN = 4;
...
function truncateForModel(value = '', maxChars = 6000) {
...
async function fetchPage(url = '') {
  try {
    const res = await fetchWithTimeout(url, { headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'text/html,application/xhtml+xml' } }, 8000);
    if (!res.ok) return { url, html: '', text: '', status: res.status };
    const contentType = String(res.headers.get('content-type') || '').toLowerCase();
    const contentLength = Number(res.headers.get('content-length') || 0);
    if (contentType && !contentType.includes('text/html')) return { url, html: '', text: '', status: res.status };
    if (contentLength && contentLength > MAX_HTML_BYTES) return { url, html: '', text: '', status: res.status };
    let html = await res.text();
    if (html.length > MAX_HTML_BYTES) html = html.slice(0, MAX_HTML_BYTES);
    return { url, html, text: decodeHtmlBasic(html).slice(0, 16000), status: res.status };
  } catch {
...
async function fetchPdfText(url = '') {
  try {
    const res = await fetchWithTimeout(url, { headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'application/pdf,*/*' } }, 10000);
    if (!res.ok) return '';
    const contentType = String(res.headers.get('content-type') || '').toLowerCase();
    const contentLength = Number(res.headers.get('content-length') || 0);
    if (contentType && !contentType.includes('pdf')) return '';
    if (contentLength && contentLength > MAX_PDF_BYTES) return '';
...
async function discoverSiteStructure(startUrl, companyName = '', options = getAnalysisOptions()) {
  ...
  for (const link of keyPages.slice(0, 6)) {
...
async function reviewLinkedEntity(entity, options = getAnalysisOptions()) {
  ...
  const entityNotes = [];
  for (const page of site.pages.slice(0, 2)) {
    entityNotes.push(...extractStructuredEntityNotes(page.text, page.url, 'linked-site'));
  }
  const docs = options.reports ? await enrichDocuments(site.candidate_documents.slice(0, 2), options) : [];
  const operationalFacts = options.operationalFootprint
    ? site.pages.flatMap(p => extractOperationalFacts(p.text, p.url, 'linked-site')).slice(0, 4)
    : [];
  return {
    ...entity,
...
function mergeSignalsWithWebsiteEvidence(signals = [], docs = [], rootSite = null) {
  ...
  if (rootSite) {
    for (const p of rootSite.page_buckets.press.slice(0, 4)) {
      websiteSignals.push({
        signal: p.title,
        date: '',
...
function getNextLevelOptions(input = {}) {
  const defaults = { persistentSystem: true, commercialDashboard: true, outboundEngine: true };
  if (!input || typeof input !== 'object') return defaults;
  const merged = {
    persistentSystem: !!input.persistentSystem,
    commercialDashboard: !!input.commercialDashboard,
    outboundEngine: !!input.outboundEngine
  };
  return Object.values(merged).some(Boolean) ? merged : defaults;
}
function stableHash(value) {
  return crypto.createHash('sha1').update(JSON.stringify(value)).digest('hex');
}
function buildAnalysisCacheKey({ company_name = '', website_url = '', persona_level = 'manager', analysis_options = {}, next_level_options = {} }) {
  return stableHash({
    company_name: String(company_name || '').trim().toLowerCase(),
    website_url: normalizeDomainLike(website_url || ''),
    persona_level,
    analysis_options,
    next_level_options
  });
}
function getEntityHistoryKey(companyName = '', websiteUrl = '') {
  const domain = normalizeDomainLike(websiteUrl || '');
  const company = normalizedText(companyName || '');
  return domain || company || 'unknown-entity';
}
function getCachedAnalysis(cacheKey) {
  const entry = ANALYSIS_CACHE.get(cacheKey);
  if (!entry) return null;
  if (Date.now() - entry.created_at > ANALYSIS_CACHE_TTL_MS) {
...
function setCachedAnalysis(cacheKey, value) {
  ANALYSIS_CACHE.set(cacheKey, { created_at: Date.now(), value });
}
function cleanupAsyncJobs() {
  const now = Date.now();
  for (const [jobId, job] of ANALYSIS_JOBS.entries()) {
    if (now - job.created_at > ASYNC_JOB_TTL_MS) ANALYSIS_JOBS.delete(jobId);
  }
}
function recordAnalysisHistory(entityKey, result) {
  const history = ANALYSIS_HISTORY.get(entityKey) || [];
  history.unshift({ created_at: new Date().toISOString(), result });
  ANALYSIS_HISTORY.set(entityKey, history.slice(0, 10));
  return history;
}
function extractSignalIdentity(item = {}) {
  return normalizeHeadlineForDedupe(item.signal || item.title || '');
}
function extractNoteIdentity(item = {}) {
  return noteDuplicateKey(item.note || item.statement || '');
}
function computeChangeDetection(previousEntry, currentResult) {
  if (!previousEntry || !previousEntry.result) {
    return {
      status: 'first_run',
      changes_detected: [],
      summary: 'No prior run found for this entity. Future runs will highlight what changed.'
    };
  }
  const previous = previousEntry.result;
  const changes = [];
  const currentSignals = new Set((currentResult.recent_signals || []).map(extractSignalIdentity).filter(Boolean));
  const previousSignals = new Set((previous.recent_signals || []).map(extractSignalIdentity).filter(Boolean));
  for (const item of (currentResult.recent_signals || [])) {
    const key = extractSignalIdentity(item);
    if (key && !previousSignals.has(key)) changes.push({ type: 'new_signal', title: item.signal || item.title || 'New signal', source_url: item.source_url || '', confidence: item.confidence || 'Medium' });
  }
  const currentReports = new Set((currentResult.corporate_reports || []).map(x => normalizeForDedupe(`${x.title} ${x.year || ''}`)).filter(Boolean));
  const previousReports = new Set((previous.corporate_reports || []).map(x => normalizeForDedupe(`${x.title} ${x.year || ''}`)).filter(Boolean));
  for (const item of (currentResult.corporate_reports || [])) {
    const key = normalizeForDedupe(`${item.title} ${item.year || ''}`);
    if (key && !previousReports.has(key)) changes.push({ type: 'new_report', title: item.title || 'New report', source_url: item.url || '', confidence: item.confidence || 'High' });
  }
  const currentEntities = new Set((currentResult.linked_entities_review || []).map(x => normalizeForDedupe(x.entity_name || x.website || '')).filter(Boolean));
  const previousEntities = new Set((previous.linked_entities_review || []).map(x => normalizeForDedupe(x.entity_name || x.website || '')).filter(Boolean));
  for (const item of (currentResult.linked_entities_review || [])) {
    const key = normalizeForDedupe(item.entity_name || item.website || '');
    if (key && !previousEntities.has(key)) changes.push({ type: 'new_linked_entity', title: item.entity_name || item.website || 'New linked entity', source_url: item.website || '', confidence: item.entity_confidence >= 70 ? 'High' : 'Medium' });
  }
  const currentNotes = new Set((currentResult.company_identity?.entity_notes || []).map(extractNoteIdentity).filter(Boolean));
  const previousNotes = new Set((previous.company_identity?.entity_notes || []).map(extractNoteIdentity).filter(Boolean));
  if (currentNotes.size > previousNotes.size) changes.push({ type: 'expanded_entity_notes', title: 'Additional supporting entity evidence identified', source_url: '', confidence: 'Medium' });
  return {
    status: changes.length ? 'changes_detected' : 'no_material_change',
    changes_detected: changes.slice(0, 10),
    summary: changes.length ? `Detected ${changes.length} change(s) versus the prior run.` : 'No material differences were detected versus the prior run.'
  };
}
function deriveSalesInsightLayer(result = {}, persona = 'manager') {
  const signals = result.recent_signals || [];
  const operationalTopics = (result.operational_footprint?.topics || []).map(x => x.topic);
  const hypotheses = [];
  const discoveryQuestions = [];
  const fitThemes = [];
  if (signals.some(x => /funding|acquisition|merger|expansion|facility|plant/i.test(`${x.category} ${x.signal}`))) {
    hypotheses.push('The company may be managing growth, footprint change, or post-transaction operational complexity.');
    discoveryQuestions.push('How are you maintaining inventory visibility and process consistency as the business expands or changes footprint?');
    fitThemes.push('Multi-site inventory control and standardized workflows');
  }
  if (signals.some(x => /erp|technology|system|automation|migration|implementation/i.test(`${x.category} ${x.signal}`))) {
    hypotheses.push('There may be active technology modernization or process digitization underway.');
    discoveryQuestions.push('Where are current systems still creating manual work, latency, or handoff risk across operations?');
    fitThemes.push('Composable modernization without forcing a full rip-and-replace motion');
  }
  if (operationalTopics.some(x => /warehouse|manufacturing|distribution|operations|supply chain/i.test(x))) {
    hypotheses.push('Operational footprint signals suggest execution complexity across inventory, fulfillment, service, or production flows.');
    discoveryQuestions.push('Which operating area feels hardest to scale today without adding headcount or buffer inventory?');
    fitThemes.push('Operational visibility, process orchestration, and inventory accuracy');
  }
  const urgency = persona === 'executive'
    ? 'Frame the conversation around resilience, growth readiness, working capital, and speed to value.'
    : persona === 'director'
      ? 'Frame the conversation around KPI lift, throughput, exceptions, and decision latency.'
      : 'Frame the conversation around daily friction, labor effort, and process consistency.';
  return {
    what_this_means: uniqueStrings(hypotheses).slice(0, 4),
    where_cloud_inventory_fits: uniqueStrings(fitThemes).slice(0, 4),
    suggested_discovery_questions: uniqueStrings(discoveryQuestions).slice(0, 4),
    messaging_guidance: urgency
  };
}
function deriveNextLevelBlueprints(result = {}, nextLevelOptions = {}) {
  const blueprints = [];
  if (nextLevelOptions.persistentSystem) {
    blueprints.push({
      option: 'Persistent company intelligence system',
      summary: 'Store company runs, evidence, and change history so the team can monitor accounts over time.',
      recommended_components: ['history store', 'scheduled refresh', 'alert subscriptions', 'entity watchlists'],
      why_now: 'This account already benefits from change tracking and repeated analysis.'
    });
  }
  if (nextLevelOptions.commercialDashboard) {
    blueprints.push({
      option: 'Commercial-grade dashboard',
      summary: 'Promote the analysis experience into a searchable dashboard with company profiles, filters, and KPI-style signal cards.',
      recommended_components: ['dashboard home', 'company profile pages', 'saved searches', 'confidence visualizations'],
      why_now: 'The current analysis payload already supports a richer dashboard view.'
    });
  }
  if (nextLevelOptions.outboundEngine) {
    blueprints.push({
      option: 'Integrated outbound engine',
      summary: 'Generate ready-to-send outreach using the intelligence package, role-based messages, and discovery prompts.',
      recommended_components: ['email draft generation', 'LinkedIn opener generation', 'call opener and voicemail scripts', 'CRM handoff notes'],
      why_now: 'The signal, pain, and buying-group layers already support outbound personalization.'
    });
  }
  return blueprints;
}
function deriveOutboundRecommendations(result = {}, companyName = '') {
  const firstQuestion = (result.smart_first_questions || [])[0] || 'Where is operational complexity creating the most friction today?';
  const whyNow = (result.why_this_matters_now || [])[0] || 'Recent public signals suggest operational change worth exploring.';
  return {
    email_subject: companyName ? `${companyName}: possible operations and inventory discussion` : 'Possible operations and inventory discussion',
    email_opener: `${whyNow} I thought it may be useful to compare how your team is currently managing visibility, manual work, and execution consistency across inventory-related flows.`,
    linkedin_opener: `I noticed a few signals that suggest change across your operation. Curious whether inventory visibility, process consistency, or system handoffs are priorities right now.`,
    call_opener: `I did some preparation on your business and saw a few indicators of operational change. ${firstQuestion}`
  };
}
function formatExportBundle(result = {}) {
  const lines = [];
  lines.push(`# ${result.company_identity?.official_company_name || result.company_identity?.legal_entity_name || 'Company Intelligence Brief'}`);
  lines.push('');
  lines.push(`Version: ${APP_VERSION}`);
  lines.push('');
  lines.push('## Company Overview');
  lines.push(result.company_overview || '');
  lines.push('');
  lines.push('## Recent Signals');
  for (const item of (result.recent_signals || []).slice(0, 10)) lines.push(`- ${item.date || 'Undated'} — ${item.signal || ''} (${item.confidence || 'n/a'})`);
  lines.push('');
  lines.push('## Sales Insight Layer');
  for (const item of (result.sales_insight_layer?.what_this_means || [])) lines.push(`- ${item}`);
  return lines.join('\n');
}
async function runCompanyInsight(body = {}) {
  if (!OPENAI_API_KEY) {
    const err = new Error('OPENAI_API_KEY not configured');
    err.statusCode = 400;
    throw err;
  }
  const { company_input, company_name, website_url, persona_level, analysis_options, next_level_options } = body || {};
  ...
  const nextOptions = getNextLevelOptions(next_level_options);
  const cacheKey = buildAnalysisCacheKey({ company_name: effectiveCompanyName, website_url: effectiveWebsiteUrl, persona_level, analysis_options: options, next_level_options: nextOptions });
  const cached = getCachedAnalysis(cacheKey);
  if (cached) {
    return { ...cached, cache: { hit: true, key: cacheKey, ttl_ms: ANALYSIS_CACHE_TTL_MS } };
  }
  const [newsItems, entityIntel] = await Promise.all([
    withTimeout(fetchAllSignals(searchEntity), 12000, []),
    withTimeout(fetchCompanyEntityIntelligence(lookupSeed), ENTITY_INTEL_TIMEOUT_MS, {
      official_domain: normalizeDomainLike(effectiveWebsiteUrl || ''),
      official_company_name: effectiveCompanyName || '',
      legal_entity_name: '',
      parent_entities: [],
      subsidiary_entities: [],
      entity_notes: []
  ...
  const deepDive = await withTimeout(runDeepDive(searchEntity, entityIntel, options), DEEP_DIVE_TIMEOUT_MS, {
  ...
  const mergedSignals = mergeSignalsWithWebsiteEvidence(newsItems, deepDive.corporate_reports || [], deepDive.root_site_data || null);
  const instructions = `
...
`;
  ...
  try {
    ...
    try {
      ...
      const e = new Error(isAbortError(retryErr) ? 'The company analysis timed out while preparing the AI summary. Try unchecking some deep-dive options or rerun the search.' : retryErr.message);
      e.statusCode = isAbortError(retryErr) ? 504 : (retryErr.statusCode || 500);
  ...
  parsed.next_level_options = nextOptions;
  ...
  parsed.sales_insight_layer = deriveSalesInsightLayer(parsed, persona_level || 'manager');
  parsed.next_level_blueprints = deriveNextLevelBlueprints(parsed, nextOptions);
  parsed.outbound_recommendations = deriveOutboundRecommendations(parsed, effectiveCompanyName || entityIntel.official_company_name || '');
  parsed.export_bundle = formatExportBundle(parsed);
  parsed.generated_at = new Date().toISOString();
  parsed.version = APP_VERSION;
  const historyKey = getEntityHistoryKey(effectiveCompanyName || entityIntel.official_company_name, effectiveWebsiteUrl || entityIntel.official_domain);
  const previousHistory = (ANALYSIS_HISTORY.get(historyKey) || [])[0] || null;
  parsed.change_detection = computeChangeDetection(previousHistory, parsed);
  const history = recordAnalysisHistory(historyKey, parsed);
  parsed.history_summary = { runs_stored: history.length + 1 > 10 ? 10 : history.length + 0, entity_key: historyKey };
  const response = { ok: true, result: parsed, cache: { hit: false, key: cacheKey, ttl_ms: ANALYSIS_CACHE_TTL_MS } };
  setCachedAnalysis(cacheKey, response);
  return response;
}

app.get('/healthz', (req, res) => res.json({ ok: true }));
app.get('/api/config', (req, res) => res.json({ pain_points: PAIN_POINTS, modelConfigured: !!OPENAI_API_KEY, model: OPENAI_MODEL, version: APP_VERSION, supports_async: true }));

app.post('/api/company-insight', async (req, res) => {
  try {
    const response = await runCompanyInsight(req.body || {});
    res.json(response);
  } catch (err) {
    return safeJsonError(res, err.statusCode || 500, err.message || 'Unknown server error', err.detail ? { detail: err.detail } : {});
  }
});

app.post('/api/company-insight-async', async (req, res) => {
  try {
    cleanupAsyncJobs();
    const jobId = `job_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    ANALYSIS_JOBS.set(jobId, { id: jobId, created_at: Date.now(), status: 'queued', progress: 5, error: '', result: null });
    Promise.resolve().then(async () => {
      ...
      try {
        ...
        job.status = 'failed';
        job.progress = 100;
        job.error = err.detail || err.message || 'Unknown server error';
      }
    });
    return res.json({ ok: true, job_id: jobId, status: 'queued' });
  } catch (err) {
    return safeJsonError(res, err.statusCode || 500, err.message || 'Unknown server error');
  }
});

app.get('/api/company-insight-async/:jobId', (req, res) => {
  cleanupAsyncJobs();
  const job = ANALYSIS_JOBS.get(req.params.jobId);
  if (!job) return safeJsonError(res, 404, 'Job not found');
  return res.json({ ok: true, job_id: job.id, status: job.status, progress: job.progress, error: job.error, result: job.result || null, cache: job.cache || null });
});
...
- The transition statement should be conversational and include:
  ...
  3. a proposal to investigate further in a follow-up meeting.`;
    ...
    return safeJsonError(res, 500, err.message || 'Unknown server error');
  }
});
